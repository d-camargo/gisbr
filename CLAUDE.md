...
> **ATUALIZADO 2026-07-02:** a `feat/diagnostico-plano-diretor` foi **mergeada na `main`** (merge commit; árvore da main = feat). A `main` agora **é** o plugin completo (diagnóstico + i18n), não mais só o espelho geobr. A branch `feat` segue no remoto como backup. Detalhe de cada tarefa (T-003…T-029) em `ACTIONS.md`; desenho em `docs/diagnostico-plano-diretor/ARQUITETURA.md`. **Submissão ao QGIS: a 0.3.0 foi BLOQUEADA pelo scan de segurança** (o dir `scratch/` vazou pro zip; ver §10); corrigido na **0.3.1**, publicada e aprovada. **0.3.2 (2026-07-06):** OSM municipal via Overpass + fix SSL do IPEA no Windows + sai do `experimental`.
...
- **Erro de SSL no Windows / OSGeo4W (IPEA) — "Unable to find issuer certificate" sem enfraquecer a segurança:** o QGIS para Windows (build OSGeo4W) muitas vezes não possui ou não confia na raiz `USERTrust RSA Certification Authority` ou nos certificados intermediários da Sectigo usados pelo IPEA, falhando no download de metadados e GPKGs. **Regra: nunca desabilitar verificação SSL (`verify=False` ou `VerifyNone`).** A solução é injetar o certificado intermediário do IPEA (empacotado em `core/certs/ipea_chain.pem`) na `QNetworkRequest` via `QSslConfiguration.addCaCertificates(...)` mantendo a validação de segurança. Adicionalmente, caso todas as conexões falhem (ex.: proxy corporativo / MITM), o plugin possui uma cópia local vendorizada em `core/data/metadata_1.7.0_gpkg.csv` como fallback final com aviso amigável.
- **Compatibilidade QGIS 3.x/4.x (PyQt5/PyQt6) — QVariant e enums escopados:** Em PyQt6/QGIS 4.x, a classe `QVariant` foi removida, e expressões como `QVariant.String` ou `QVariant.LongLong` quebram em tempo de execução. Para retrocompatibilidade, use a função utilitária `field_type(kind)` definida em `core/qgis_compat.py` (que faz o mapeamento transparente entre `QVariant` e `QMetaType.Type`). Além disso, os enums no PyQt6 são estritamente escopados (por exemplo, use `Qt.DockWidgetArea.RightDockWidgetArea` e `Qt.CheckState.Unchecked` em vez dos antigos caminhos globais do PyQt5). Todo código novo deve aderir à regra de "enums sempre escopados" para evitar exceções em ambas as versões.
