...
Open the **GISBR** panel (toolbar button / *Plugins → GISBR*). Pick a **state → municipality** (or switch to **Metropolitan region** mode to select one of the 84 IBGE-cataloged RMs), choose the sources you want (checkboxes grouped by axis), a destination **GeoPackage**, and click **Load**. GISBR downloads each source **filtered to that municipality or RM**, clips it to the corresponding polygon, saves one layer per source in the GeoPackage, and adds them to the project. In RM mode, layers are suffixed `_rm<id>` (e.g. `sicar_imoveis_rm04501`) and a unified boundary layer `rm_<id>` is saved alongside; OSM sources (road network and POIs) are municipality-only — load them per city and they coexist in the same GeoPackage.
...
Abra o painel **GISBR** (botão na barra / *Complementos → GISBR*). Escolha **UF → Município** (ou alterne para o modo **Região metropolitana** para selecionar uma das 84 RMs catalogadas pelo IBGE), marque as fontes desejadas (checkboxes agrupados por eixo), um **GeoPackage** de destino e clique em **Carregar**. O GISBR baixa cada fonte **filtrada pelo município ou RM**, recorta pelo polígono correspondente, grava uma camada por fonte no GeoPackage e adiciona ao projeto. No modo RM, as camadas recebem o sufixo `_rm<id>` (ex.: `sicar_imoveis_rm04501`) e uma camada de limite unificado `rm_<id>` é gravada junto; as fontes OSM (rede viária e POIs) funcionam apenas por município — carregue-as por cidade e elas convivem no mesmo GeoPackage.
