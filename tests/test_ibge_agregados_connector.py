...
def test_fetch_layer_sucesso_real_fixture(monkeypatch):
    json_bytes = b"""[
  {
    ...
    # [code_muni, var_id, var_nome, unidade, produto, periodo, valor]
    assert attrs[0] == "3170404"
    assert attrs[1] == "109"
    assert attrs[2] == "Area plantada"
    assert attrs[3] == "Hectares"
    assert attrs[4] == "Soja (em grao)"
    assert attrs[5] == "2023"
    assert attrs[6] == 185000.0
...
def test_build_url_multiplas_localidades():
    url = build_url(1612, ["3106200", "3170404"], variaveis=["109"], periodo=2023)
    assert "https://servicodados.ibge.gov.br/api/v3/agregados/1612/periodos/2023/variaveis/109" in url
    assert "localidades=N6[3106200,3170404]" in url


def test_fetch_layer_multiplas_localidades(monkeypatch):
    json_bytes = b"""[
  {
    "id": "109",
    "variavel": "Area plantada",
    ...
    "resultados": [
      {
        ...
        "series": [
          ...
          {
            "localidade": { "id": "3170404", "nome": "Unai (MG)" },
            "serie": { "2023": "185000" }
          }
        ]
      }
    ...
    layer = fetch_layer(1612, ["3106200", "3170404"], "pam_rm", variaveis=["109"], periodo=2023)
    assert layer.isValid()
    ...
    codes = [f["code_muni"] for f in feats]
    assert "3106200" in codes
    assert "3170404" in codes
